...
# Temporary removed this import -test
...
url='http://10.213.17.21/api/ipam/ip-addresses/'
...
list = res_json["results"]
ip_list_netbox = [sub['address'] for sub in list]
id_list = [sub['id'] for sub in list]
print(id_list)
#print(ip_list_netbox)

# GET ID from Given IP
id = [x['id'] for x in list if x['address'] == "10.213.17.10/24"]
...
# remove \n from nmap_ip_list and update it with /24
new_nmap_list = []
for ip in list_of_nmap_ip:
...
list_to_del = [item for item in new_ip_list if item not in new_nmap_list]
print("differnet between the 2 list to DELETE\n")
print(list_to_del)
print("number of unuiqe ip's {}".format(len(list_to_del)))


list_to_add = [item for item in new_nmap_list if item not in new_ip_list]
